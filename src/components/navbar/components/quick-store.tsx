...
import StoreIcon from 'src/assets/common/store';
...
const QuickStore = ({ modals }) => {
    const dispatch = useDispatch();
    const storeRef = useRef<any>();
    const [openStoreBar, setOpenStoreBar] = useState(false);
    ...
    useEffect(() => {
        ...
        function handleClickOutside(event) {
            if (!storeRef.current?.contains?.(event.target) && openStoreBar) {
                setOpenStoreBar(false);
            } else setOpenStoreBar(true);
        }
    ...
    return (
        <Box
            ...
            sx={{
                ...
                borderRadius: '9px',
                paddingBottom: '2px',
                backgroundColor: modals.find((item) => item.id === 'store-quick')
                    ? '#6FCBDF6e'
                    : 'unset'
            }}>
            <StoreIcon />
...
export default connect(mapStateToProps)(QuickStore);
    ...
    return (
        <Box width="100%" height="100%">
            <Box>
                ...
                {notifs?.length < 1 && (
                    <Typography color="gray">There are no products yet!</Typography>
                )}
                {/* {mustShowLoadMore && (
                    <LoadBtn
